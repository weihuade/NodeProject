window.onload = function() {
	...
	xhr1.onreadystatechange = function() {
		if(xhr1.readyState == 4 && xhr1.status == 200) {
			let length = JSON.parse(xhr1.responseText).data.length;
			console.log(length)
			alldata();
			...
			morejiazai.onclick = function() {
				...
				xhr2.onreadystatechange = function() {
					if(xhr2.readyState == 4 && xhr2.status == 200) {
						let arr = JSON.parse(xhr2.responseText);
						xuanlan(arr);
						var num = Math.ceil(length * 1 / arr.qty * 1)
						if(arr.page * 1 >= num) {
	...
	xhr1.open('get', '/bbhome/selectall', true)
